...
Students are enrolled in a Moodle course in which the programming exercise is available.
"""
...
class Student(Base):
    ...
    grade=Column(String)
    oralexam_time=Column(DateTime)

    # foreign key relationship to table Submission
    ...
    def get_students_all(cls):
        ...
        If the script has been run already, the table contains all people enrolled in the Moodle course.
        Parameters: None
        Returns:
    ...
    def get_or_insert(cls, name, mid):
        """
        Insert a new student into the table "Student" or get the corresponding Student object if the student already exists.
        First tries to find student with name, then with their moodle ID.
        If there are multiple students with the same name, the first one is selected.
        Parameters:
        ...
        if student in [None, [], [[]]]:
            student=dbm.session.query(Student)\
                .filter_by(moodle_id=mid)\
                .order_by(Student.id).first()
        if student in [None, [], [[]]]:
            # Warn("Student with name "+str(name)+" did not exist. A database entry is now created.")
        ...
        if type(student)==list:
            logging.error(
                f"Student was not found but there are students with similar names. Selecting {student[0].name}")
            return student[0]
        return student
    ...
    def get_student_by_name(cls, student_name):
        """
        Gets the student only using the students name.
        If no student with the exact name was found, at most 5 other students with a similar name are returned.
        Parameters:
    ...
    @classmethod
    def get_student_by_moodle_id(cls, student_moodle_id):
        """
        ...
        """
        result=dbm.session.query(Student)\
            .filter_by(moodle_id=student_moodle_id)\
            .order_by(Student.id).first()
        return result
    ...
    def get_student_by_submission(cls, submission):
        ...
        result=dbm.session.query(Student)\
            .join(sub.Submission)\
            .filter(Student.id==submission.student_id).first()
        return result
    ...
    def is_student_passed(cls, name):
        ...
        results=dbm.session.query(Student)\
            ...
            .filter(r.Run.passed==True, Student.name==name).count()
        if results>0:
            return True
        return False
    ...
    def get_students_passed(cls):
        ...
        # results=dbm.session.query(Student,Submission, Run)
        # .join(Submission, Submission.student_id==Student.id)
        # .join(Run, Run.submission_id==Submission.id)
        # .filter_by(passed=True).group_by(Student.name).all()
    ...
    def get_students_not_passed(cls):
        ...
        Parameters: None
        Returns:
            List of Student objects where the student has handed in at least one submission
            but which do not have a submission with a passed run.
        
        
        """
        passed=cls.get_students_passed()
        submitted_once=dbm.session.query(Student)\
            .join(sub.Submission, sub.Submission.student_id==Student.id)\
    ...
    @classmethod
    def get_oralexam_done(cls):
        """
        Get a list of all students that have passed their oral exam.
        Parameters: None
        Returns:
