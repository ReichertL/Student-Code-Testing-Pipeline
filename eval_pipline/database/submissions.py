...
Submissions are solutions handed in by the students proposing a solution for the given programming exercise.
Submissions are handed in via Moodle and need to be regularly retried from the Moodle servers.
"""
...
class Submission(Base):
    ...
    def insert_submission(cls, student, path, time):
        """
        Insert new submission in table "Submission" if it does not already exist.
        ...
        Returns:
            Nothing.
        """
        count=dbm.session.query(Submission)\
            .filter(Submission.student_id==student.id, Submission.submission_path==path, Submission.submission_time==time)\
            .count()
    ...
    def get_not_checked(cls):
        ...
        """
        submissions=dbm.session.query(Submission, Student)\
            .join(Student) \
    ...
    def get_not_checked_for_name(cls, student_name):
        ...
        """
        submissions_student=dbm.session.query(Submission, Student)\
            .join(Student).filter(Student.name==student_name, Submission.is_checked!=True).all()
    ...
    def get_last_for_name(cls, name):
        ...
        results=dbm.session.query(Submission, Student).join(Student) \
            .filter(Submission.is_checked==True, Student.name==name)\
    ...
    def get_passed_for_name(cls, name):
        ...
        """
        results=dbm.session.query(Submission)\
            .join(Student)\
            .join(Run, Run.submission_id==Submission.id) \
            .filter(Run.passed==True, Student.name==name)\
    ...
    def is_passed(cls, sub):
        """
        Returns for a submission if it is passed. WARNING : Might not work. Use corresponding function in Run.py instead.
        Parameters:
        ...
        """
        results=dbm.session.query(Submission)\
            .join(Run, Run.submission_id==sub.id)\
            .filter(Run.passed==True).count()
        if results>0:
            return True
        return False

    @classmethod
    def get_submissions_for_notification(cls):
        """
        ...
        Returns:
            List of pairs each containing a Submission and  a Student objects
        """
        results=dbm.session.query(Student, Submission)\
            .join(Submission, Submission.student_id==Student.id)\
            .filter(Submission.is_checked==True) \
            .filter(or_(Submission.student_notified==False, Submission.student_notified==None)).all()
        return results

    @classmethod
    def get_submissions_for_notification_by_name(cls, name):
        """
        Get all submission by a specific student which are already checked, but no notification has been sent out via Moodle.
        Parameters: 
